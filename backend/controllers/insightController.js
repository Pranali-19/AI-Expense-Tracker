...
import { generateBudgetAlert, generateMonthlyInsight, generateSavingTips } from '../utils/gemini.js';
...
const buildSavingTips = async (userId) => {
    ...
    const content = await generateSavingTips({
        topcategories : top.rows.map( r => ({
            ...
            transactionsCount: parseInt(r.count, 10),
        })),
...
const buildBudgetAlert = async (userId , categoryId) => {
    if (!categoryId) {
        const err = new Error('catergoryId is required for budget_alert');
        err.status = 400;
        throw err;
    }
...
export const generateInsight = async (req, res) => {
    ...
    try {
        ...
        console.error('GenrateInsights error' , error);
