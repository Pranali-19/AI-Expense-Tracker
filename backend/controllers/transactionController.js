...
export const getTransactions = async (req , res) => {
    ...
    if (search) {
        conditions.push(`t.description ILIKE $${idx} OR t.notes TLIKE $${idx}`);
        values.push(`%${search}%`);
        idx++;
    }
...
export const getTransationById = async (req , res) => {
    console.log('🔥 getTransationById called with:', req.params);
    ...
    try {
        ...
        console.error('GetTransationById error:', error);
        res.status(500).json({ message: 'Server error'})
    }
};
...
export const updateTransaction = async (req , res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Transcation not found' });
        }
        res.json(result.rows[0]);
    } catch (error) {
        console.error('updateTranaction error:' ,error);
        res.status(500).json({ message: "Server error" });
    }
};
