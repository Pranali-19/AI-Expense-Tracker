// import dotenv from 'dotenv';
// import { GoogleGenAI} from '@google/genai'


// dotenv.config();

// const ai = new GoogleGenAI({ apiKey: process.env.GEMENI_APP_KEY});

// if(!process.env.GEMENI_APP_KEY) {
//     console.error(' WARNING:GEMENI_APP_KEY IS not set. Ai feature will not not work')
// }

// const stripMarkdown = (text) => {
//     let cleaned = text.trim();
//     if(cleaned.startsWith('```json')) {
//         cleaned = cleaned.replace(/```json\n?/g, '').replace(/```\n?$/g, '');
//     } else if (cleaned.startsWith('```')){
//         cleaned = cleaned.replace(/```\n?/g, '');
//     }
//     return cleaned.trim();
// };

// export const generateMonthlyInsight = async ({
//     totalIncome,
//     totalExpenses,
//     savingRate,
//     expenseBreakdown,
//     previousMonths,
//     currency = 'USD',
// }) => {
//     const breakdownText = expenseBreakdown.length > 0
//         ? expenseBreakdown.map(c => `-${c.category}: ${currency} ${c.amount.toFixed(2)}`).join('\n')
//         : '- No expenses recorded yet';
    
//     const trendText = previousMonths.length > 0 
//         ? previousMonths.map(m => `- ${m.month}: Income ${currency} ${m.income.toFixed(2)}, Expenses ${currency} ${m.expenses.toFixed(2)}`)
//         : '- NO previous month data available';
    
//     const prompt = `Analyze this user's monthly financial data and generate actionable insights.
// Currency: ${currency}
// Total Income (this month): ${currency}  ${totalIncome.toFixed(2)}
// Total Expenses (this month): ${currency} ${totalExpenses.toFixed(2)}
// Savings Rate: ${savingRate.toFixed(2)}%

// Expense breakdown by category (this month):
// ${breakdownText}

// Previous months trend:
// ${trendText}

// Return ONLY valid JSON (no markdown , no commentary) in this exact structure:
// {
//     "summary": "2-3 sentence summary of the user's finacial health this month",
//     "hightlight": ["Postive observation 1", "Positive obeservation 2"],
//     "concerns": ["Concerns 1" , "Concerns 2"],
//     "recommendations" : [
//         {"title":"short title" , "detail": "Actionable suggestion (1-2 sentence)"}
//     ],
//     "topSpendingCategory": "Category name or null",
//     "estimatedMonthlySaving": number,
//     "HealthScore":number
// }

// Constraints:
// - "Healthcare" must be an integer between 0 and 100.
// - "provide three recommendations.
// - Reference actual number from the data. Tone: friendly but honest.`;

//     try {
//         const response = await ai.models.generateContent({
//             model: 'gemini-2.5-flash',
//             contents: prompt,
//         });
//         const cleaned = stripMarkdown(response.text);
//         return JSON.parse(cleaned);
//     } catch (error) {
//         console.error("Gemini API error (monthly insight):" , error);
//         throw new Error("Failed to generate monthly insight.Please try again.");
//     }
// };

// export const generateBudgetAlert = async (
//     categoryName,
//     budgetAmount,
//     spendAmount,
//     daysIntoPeriod,
//     totalPeriodDays,
//     currency = "USD"
// ) => {
//     const percentUsed = ((spendAmount / budgetAmount) * 100).toFixed(1);
//     const daysLeft = totalPeriodDays - daysIntoPeriod;

//     const prompt = ` A user is tracking a budget. generate a helpful alert.

// Category: ${categoryName}
// Budget: ${currency} ${budgetAmount.toFixed(2)}
// Spent so far:${currency} ${spendAmount.toFixed(2)} (${percentUsed}% used)
// Days into period: : ${daysIntoPeriod} of ${totalPeriodDays} (${daysLeft} days remaining)

// Return ONLY the valid JSON (no markdown):
// {
//    "severity": "info|warning| critical",
//    "title": "short alert title",
//    "message": "1-2 sentence empathetic message referring actual numbers",
//    "suggestions": ["specific action 1" , "specific action 2" , "specific action 3"]
// }

// Severity guide:
// - info: under 70% spent
// - warning: 70-100% spent
// -critical: over 100% spent `;

//     try {
//         const response = await ai.models.generateContent({
//             model: 'gemini-2.5-flash',
//             contents: prompt,
//         });
//         const cleaned = stripMarkdown(response.text);
//         return JSON.parse(cleaned);
//     } catch (error) {
//         console.error("Gemini API error (budget alert):" , error);
//         throw new Error("Failed to generate budget alert.Please try again.");
//     }
// };

// export const generateSavingTips = async ({ topcategories , monthlyIncome ,currency = 'USD'}) => {
//     const categoryText = topcategories.length > 0
//         ? topcategories.map( c => `- ${c.category}: ${currency} ${c.amount.toFixed(2)} across ${c.transactionCount}`)
//         : '- No spending data available' ;

//         const prompt = `Generate personalize savings tips for a user.

// Monthly Income (last 30 days): ${currency} ${monthlyIncome.toFixed(2)}
// Top spending categories (last 30 days): ${categoryText}

// Return ONLY valid JSON (no markdown):
// {
//     "overallTip": "Top-level 1-sentence targets",
//     "tips" : [
//       {  
//         "category": "Category this targets",
//         "title": "Short tip title",
//         "details": "2-3 sentence actionable suggestion",
//         "estimatedSavings": number
//        }
//     ]
// }

// Provide exactly 4 tips. Each tip should reference an actual category from the data and include a realistic monthly.`
 
//      try {
//         const response = await ai.models.generateContent({
//             model: 'gemini-2.5-flash',
//             contents: prompt,
//         });
//         const cleaned = stripMarkdown(response.text);
//         return JSON.parse(cleaned);
//     } catch (error) {
//         console.error("Gemini API error (saving tips):" , error);
//         throw new Error("Failed to generate savings tips.Please try again.");
//     }
// };

// export const analyzeTransactionList = async ({ transactions , currency = 'USD'}) => {
//     const formatDate = (d) => {
//         if(!id) return '';
//         if (d instanceof Date) {
//             const y = d.getFullYear();
//             const m = String(d.getMonth() +1).padStart(2, '0');
//             const day = String(d.getDate()).padStart(2, '0');
//             return `${y}-${m}-${day}` ;
//         }
//         return String(d).split('T')[0];
//     };

//     const lines = transactions
//         .slice(0, 50)
//         .map((t) => {
//             const date = formatDate(t.transaction_date);
//             const amt = parseFloat(t.amount).toFixed(2);
//             const cat = t.category_name || 'uncategorized';
//             const desc = t.description ? ` | ${t.description}` : '';
//             return `- ${date}: ${t.type} ${currency} ${amt} | ${cat}${desc}`;
//         })
//         .join('\n');

//         const prompt = `Analyze these ${transactions.length}  transactions and provide a concise, helpful spending inclued

// Transactions: ${lines}

// Return ONLY valid JSON (no markdown): 
// { 
//    "insight": "2-4 sentence analysis with specific numbers from the data. Tone: friendly, helpful.",
//    "highlight": "Single short phrase capturing the key takeaway (e.g., 'Heavy on dining' , 'Stable income' , etc);
// } `;
 
//     try {
//         const response = await ai.models.generateContent({
//             model: 'gemini-2.5-flash',
//             contents: prompt,
//         });
//         const cleaned = stripMarkdown(response.text);
//         return JSON.parse(cleaned);
//     } catch (error) {
//         console.error("Gemini API error (analyze transcations):" , error);
//         throw new Error("Failed to analyze transcations .Please try again.");
//     }
// };

// export const analyzeBudgetList = async ({ budgets, currency = "USD"}) => {
//     const lines = budgets.map((b) => {
//         const spent = parseFloat(b.spent);
//         const total = parseFloat(b.amount);
//         const pct = total > 0 ? ((spent / total) * 100).toFixed(1) : '0';
//         return `Budget ID ${b.id} | Category: ${b.category_name} | Limit: ${currency} ${total.toFixed(2)} | Spent:`
//     }).join('\n');

//     const prompt = `You're a personal finance assistant . Analyze each budget below and provide a one-sentence .

// Today: ${new Date().toISOString().split('T')[0]}
// Budgets: ${lines}

// For each budget, return:
// -status: 'good' (well-paced, under target), 'caution' (approaching limit or above 70%), or 'concerning' (over spending),
// -message: A specific, friendly 1-sentence assessment with actionable feedback or encouragement

// Return ONLY valid JSON (no markdown):
// {
//     "analyses": [
//         { "budgetId" : number , "status" : "good"|"caution"|"concerning","message":"string"}
//     ]
// }`;
    
//     try {
//         const response = await ai.models.generateContent({
//             model: 'gemini-2.5-flash',
//             contents: prompt,
//         });
//         const cleaned = stripMarkdown(response.text);
//         return JSON.parse(cleaned);
//     } catch (error) {
//         console.error("Gemini API error (analyze budgets):" , error);
//         throw new Error("Failed to analyze budgets .Please try again.");
//     }
// };

// export default {
//     generateMonthlyInsight ,
//     generateBudgetAlert,
//     generateSavingTips,
//     analyzeTransactionList,
//     analyzeBudgetList,
// };


import dotenv from 'dotenv';
import { GoogleGenAI } from '@google/genai';

dotenv.config();

const apiKey = process.env.GEMENI_APP_KEY;

if (!apiKey) {
    console.error(
        'WARNING: GEMENI_APP_KEY is not set. AI features will not work.'
    );
}

const ai = new GoogleGenAI({
    apiKey,
});

const stripMarkdown = (text) => {
    let cleaned = String(text || '').trim();

    if (cleaned.startsWith('```json')) {
        cleaned = cleaned
            .replace(/^```json\s*/i, '')
            .replace(/```\s*$/i, '');
    } else if (cleaned.startsWith('```')) {
        cleaned = cleaned
            .replace(/^```\s*/i, '')
            .replace(/```\s*$/i, '');
    }

    return cleaned.trim();
};

const parseNumber = (value) => {
    const number = Number(value);
    return Number.isFinite(number) ? number : 0;
};


/* =========================================================
   MONTHLY INSIGHT
========================================================= */

export const generateMonthlyInsight = async ({
    totalIncome,
    totalExpenses,
    savingRate,
    expenseBreakdown = [],
    previousMonths = [],
    currency = 'USD',
}) => {
    const breakdownText =
        expenseBreakdown.length > 0
            ? expenseBreakdown
                  .map(
                      (c) =>
                          `- ${c.category}: ${currency} ${parseNumber(
                              c.amount
                          ).toFixed(2)}`
                  )
                  .join('\n')
            : '- No expenses recorded yet';

    const trendText =
        previousMonths.length > 0
            ? previousMonths
                  .map(
                      (m) =>
                          `- ${m.month}: Income ${currency} ${parseNumber(
                              m.income
                          ).toFixed(2)}, Expenses ${currency} ${parseNumber(
                              m.expense
                          ).toFixed(2)}`
                  )
                  .join('\n')
            : '- No previous month data available';

    const prompt = `
Analyze this user's monthly financial data and generate actionable insights.

Currency: ${currency}

Total Income (this month):
${currency} ${parseNumber(totalIncome).toFixed(2)}

Total Expenses (this month):
${currency} ${parseNumber(totalExpenses).toFixed(2)}

Savings Rate:
${parseNumber(savingRate).toFixed(2)}%

Expense breakdown by category:
${breakdownText}

Previous months trend:
${trendText}

Return ONLY valid JSON. Do not include markdown or commentary.

Use exactly this structure:

{
    "summary": "2-3 sentence summary of the user's financial health this month",
    "highlight": [
        "Positive observation 1",
        "Positive observation 2"
    ],
    "concerns": [
        "Concern 1",
        "Concern 2"
    ],
    "recommendations": [
        {
            "title": "Short title",
            "detail": "Actionable suggestion in 1-2 sentences"
        }
    ],
    "topSpendingCategory": "Category name or null",
    "estimatedMonthlySaving": 0,
    "healthScore": 0
}

Constraints:
- healthScore must be an integer between 0 and 100.
- Provide exactly 3 recommendations.
- Reference actual numbers from the provided data.
- Tone should be friendly but honest.
`;

    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3.6-flash',
            contents: prompt,
        });

        const cleaned = stripMarkdown(response.text);

        return JSON.parse(cleaned);
    } catch (error) {
        console.error(
            'Gemini API error (monthly insight):',
            error
        );

        throw new Error(
            'Failed to generate monthly insight. Please try again.'
        );
    }
};


/* =========================================================
   BUDGET ALERT
========================================================= */

export const generateBudgetAlert = async ({
    categoryName,
    budgetAmount,
    spentAmount,
    daysIntoPeriod,
    totalPeriodDays,
    currency = 'USD',
}) => {
    const safeBudget = parseNumber(budgetAmount);
    const safeSpent = parseNumber(spentAmount);

    const percentUsed =
        safeBudget > 0
            ? ((safeSpent / safeBudget) * 100).toFixed(1)
            : '0.0';

    const daysLeft =
        parseNumber(totalPeriodDays) -
        parseNumber(daysIntoPeriod);

    const prompt = `
A user is tracking a budget. Generate a helpful budget alert.

Category: ${categoryName}

Budget:
${currency} ${safeBudget.toFixed(2)}

Spent so far:
${currency} ${safeSpent.toFixed(2)}

Percentage used:
${percentUsed}%

Days into period:
${daysIntoPeriod} of ${totalPeriodDays}

Days remaining:
${daysLeft}

Return ONLY valid JSON. Do not include markdown.

Use exactly this structure:

{
    "severity": "info",
    "title": "Short alert title",
    "message": "1-2 sentence empathetic message referring to actual numbers",
    "suggestions": [
        "Specific action 1",
        "Specific action 2",
        "Specific action 3"
    ]
}

Severity guide:
- info: under 70% spent
- warning: 70% to 100% spent
- critical: over 100% spent
`;

    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3.6-flash',
            contents: prompt,
        });

        const cleaned = stripMarkdown(response.text);

        return JSON.parse(cleaned);
    } catch (error) {
        console.error(
            'Gemini API error (budget alert):',
            error
        );

        throw new Error(
            'Failed to generate budget alert. Please try again.'
        );
    }
};


/* =========================================================
   SAVING TIPS
========================================================= */

export const generateSavingTips = async ({
    topcategories = [],
    monthlyIncome,
    currency = 'USD',
}) => {
    const categoryText =
        topcategories.length > 0
            ? topcategories
                  .map(
                      (c) =>
                          `- ${c.category}: ${currency} ${parseNumber(
                              c.amount
                          ).toFixed(2)} across ${
                              c.transactionsCount || 0
                          } transactions`
                  )
                  .join('\n')
            : '- No spending data available';

    const prompt = `
Generate personalized savings tips for a user.

Monthly Income (last 30 days):
${currency} ${parseNumber(monthlyIncome).toFixed(2)}

Top spending categories (last 30 days):
${categoryText}

Return ONLY valid JSON. Do not include markdown.

Use exactly this structure:

{
    "overallTip": "Top-level one-sentence savings strategy",
    "tips": [
        {
            "category": "Category from the provided data",
            "title": "Short tip title",
            "details": "2-3 sentence actionable suggestion",
            "estimatedSavings": 0
        }
    ]
}

Constraints:
- Provide exactly 4 tips.
- Each tip should reference an actual category from the provided data.
- Include realistic estimated savings.
- Use actual numbers from the data where appropriate.
`;

    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3.6-flash',
            contents: prompt,
        });

        const cleaned = stripMarkdown(response.text);

        return JSON.parse(cleaned);
    } catch (error) {
        console.error(
            'Gemini API error (saving tips):',
            error
        );

        throw new Error(
            'Failed to generate savings tips. Please try again.'
        );
    }
};


/* =========================================================
   ANALYZE TRANSACTIONS
========================================================= */

export const analyzeTransactionList = async ({
    transactions = [],
    currency = 'USD',
}) => {
    const formatDate = (d) => {
        if (!d) return '';

        if (d instanceof Date) {
            const y = d.getFullYear();
            const m = String(d.getMonth() + 1).padStart(2, '0');
            const day = String(d.getDate()).padStart(2, '0');

            return `${y}-${m}-${day}`;
        }

        return String(d).split('T')[0];
    };

    const lines = transactions
        .slice(0, 50)
        .map((t) => {
            const date = formatDate(t.transaction_date);
            const amount = parseNumber(t.amount).toFixed(2);
            const category =
                t.category_name || 'Uncategorized';
            const description = t.description
                ? ` | ${t.description}`
                : '';

            return `- ${date}: ${t.type} ${currency} ${amount} | ${category}${description}`;
        })
        .join('\n');

    const prompt = `
Analyze these ${transactions.length} transactions and provide a concise, helpful spending insight.

Transactions:
${lines}

Return ONLY valid JSON. Do not include markdown.

Use exactly this structure:

{
    "insight": "2-4 sentence analysis with specific numbers from the data. Tone: friendly and helpful.",
    "highlight": "Single short phrase capturing the key takeaway"
}
`;

    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3.6-flash',
            contents: prompt,
        });

        const cleaned = stripMarkdown(response.text);

        return JSON.parse(cleaned);
    } catch (error) {
        console.error(
            'Gemini API error (analyze transactions):',
            error
        );

        throw new Error(
            'Failed to analyze transactions. Please try again.'
        );
    }
};


/* =========================================================
   ANALYZE BUDGETS
========================================================= */

export const analyzeBudgetList = async ({
    budgets = [],
    currency = 'USD',
}) => {
    const lines = budgets
        .map((b) => {
            const spent = parseNumber(b.spent);
            const total = parseNumber(b.amount);

            const pct =
                total > 0
                    ? ((spent / total) * 100).toFixed(1)
                    : '0.0';

            // return (
            //     `Budget ID ${b.id} | ` +
            //     `Category: ${b.category_name} | ` +
            //     `Limit: ${currency} ${total.toFixed(2)} | ` +
            //     `Spent: ${currency} ${spent.toFixed(2)} | ` +
            //     `Used: ${pct}%`
            // );
            return `Budget ID ${b.id} | Category: ${b.category_name} | Limit: ${currency} ${total.toFixed(2)} | Spent: ${currency} ${spent.toFixed(2)} (${pct}%)`;
        })
        .join('\n');

    const prompt = `
You are a personal finance assistant.

Analyze each budget below and provide a one-sentence assessment.

Today:
${new Date().toISOString().split('T')[0]}

Budgets:
${lines}

For each budget, return:

- good: well-paced and under target
- caution: approaching limit or above 70%
- concerning: over budget

Return ONLY valid JSON. Do not include markdown.

Use exactly this structure:

{
    "analyses": [
        {
            "budgetId": 1,
            "status": "good",
            "message": "Specific, friendly one-sentence assessment"
        }
    ]
}
`;

    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3.6-flash',
            contents: prompt,
        });

        const cleaned = stripMarkdown(response.text);

        return JSON.parse(cleaned);
    } catch (error) {
        console.error(
            'Gemini API error (analyze budgets):',
            error
        );

        throw new Error(
            'Failed to analyze budgets. Please try again.'
        );
    }
};


export default {
    generateMonthlyInsight,
    generateBudgetAlert,
    generateSavingTips,
    analyzeTransactionList,
    analyzeBudgetList,
};




